...
const PlayerMatch = ({ gameWeek, points, opponent }) => {
    const { setIsPlayerMatchMenuOpen } = useAppContext();

    const handlePlayerMatchClick = () => {
        setIsPlayerMatchMenuOpen(true);
    }
    return (
        <div onClick={handlePlayerMatchClick} className='flex flex-col gap-2 items-center bg-[#FFFFFF80] rounded-lg'>
